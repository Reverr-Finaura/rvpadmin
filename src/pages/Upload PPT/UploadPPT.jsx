...
const UploadPPT = () => {
  ...
  const handleTagoptionClick = (e) => {
    ...
    if (e.target.className === styles.tagListContainer) {
      return;
    }
    if (tagArray.includes(text)) {
      let newTagArr = tagArray.filter((item) => {
        return item !== text;
      });
  ...
  const uploadDataToFirebase = async (e) => {
    ...
    setIsloading(true);
    const thumbnailImg = await uploadMedia(
      thumbnail,
      "pptTemplates/thumbnail/"
    );
          ...
          <fieldset className={styles.form} style={{ display: "flex" }}>
            ...
            <fieldset className={styles.input}>
              ...
              <input
                ...
                className={styles.input}
                type='text'
                placeholder='https://docs.google.com/presentation/d/FILEID/edit?usp=share_link&ouid=UID&rtpof=true&sd=true'
                value={data.link}
                required
                autoComplete='off'
              />
            ...
            <fieldset className={styles.input}>
              <legend>Tag*</legend>
              <div
                onClick={handleTagoptionClick}
                className={styles.tagListContainer}
              >
                <p value='For You' className={styles.tagOptions}>
                  For You
                </p>
                <p value='Popular' className={styles.tagOptions}>
                  Popular
                </p>
                <p value='Basic' className={styles.tagOptions}>
                  Basic
                </p>
                <p value='Startups' className={styles.tagOptions}>
                  Startups
                </p>
                <p value='Business' className={styles.tagOptions}>
                  Business
                </p>
                <p value='Marketing' className={styles.tagOptions}>
                  Marketing
                </p>
                <p value='Academic' className={styles.tagOptions}>
                  Academic
                </p>
                <p value='Sales' className={styles.tagOptions}>
                  Sales
                </p>
              </div>
            ...
            {tempImgUrl ? (
              <img className={styles.image} src={tempImgUrl} alt='preview' />
            ) : null}
            {isLoading ? (
              <HourglassSplit className={styles.loadingIcon} />
            ) : (
